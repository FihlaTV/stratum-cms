	...
	if (req.session && req.session.context && req.session.context.ContextID) {
		...
		try {
			locals.registerLoggedIn =
				locals.context.Unit.Register.RegisterID.toString() ===
				locals.registerId;
		} catch (e) {
			console.log(
				'Could not parse RegisterID for locals.registerLoggedIn'
			);
		}
	}
	async.series(
		{
			...
			getRegisterInformation: function(cb) {
				keystone
					...
					.model.findOne(function(err, register) {
						if (register) {
