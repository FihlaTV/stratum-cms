	...
	async.series(
		{
				...
				async.each(
					...
					function(page, cb) {
						...
						try {
							menuBlock = page.menu;
							if (menuBlock) {
								context.paths[menuBlock.slug] =
									context.paths[menuBlock.slug] || {};
								context.paths[menuBlock.slug][page.slug] =
									'page';
							}
							cb();
						} catch (err) {
					...
					function(subPage, cb) {
						...
						try {
							...
							if (menuBlock) {
								context.paths[menuBlock.slug] =
									context.paths[menuBlock.slug] || {};
								context.paths[menuBlock.slug][subPage.slug] =
									'subpage';
							}
							cb();
						} catch (err) {
			...
			buildCompletePaths: function(next) {
				context.complete = _.flatten(
					_.map(context.paths, function(pages, menu) {
