...
const {
	CLIENT_REGISTER_ID,
	CLIENT_STRATUM_SERVER,
	NODE_ENV,
	CLIENT_DEMO_USERNAME,
	CLIENT_DEMO_PASSWORD = 'demo',
} = process.env;
...
export function changeContext(roleId, unitId, contexts) {
	return (dispatch, getState) => {
		const context = contexts.find(
			c => c.Unit.UnitID === unitId && c.Role.RoleID === roleId
		);
		if (!context) {
		...
		dispatch(setContextLoadFlag(true));
		return fetch(
			`/stratum/api/authentication/context?_=${new Date().getTime()}`,
			{
				credentials: 'include',
				method: 'PUT',
				headers: {
	...
	return dispatch => {
		...
		if (!protocol && NODE_ENV !== 'development') {
			dispatch(
				loginError(
					new Error(
						'Det går inte att logga in pga att du inte besöker webbplatsen över https. Var god försök igen under https.'
					)
				)
			);
		}
...
export function initiateSITHSLogin() {
	return dispatch => {
		...
		return fetch(
			`${CLIENT_STRATUM_SERVER}/api/authentication/login?_=${new Date().getTime()}`,
			{
				credentials: 'include',
			}
		)
			...
			.then(json => {
				if (json.success) {
					...
					const error = new Error(
						json.code ? sithsErrorMessages(json.code) : json.message
					);
					throw error;
				}
			})
...
function setUserInfo(context, contexts, initial) {
	const wrongRegister =
		context.Unit.Register.RegisterID !== parseInt(CLIENT_REGISTER_ID);
	return {
		type: SET_USER_INFO,
		...
		return fetch(`/api/authentication/login?_=${new Date().getTime()}`, {
			...
			.then(json => {
				if (json.success) {
					...
					const error = new Error(
						json.code
							? getStratumProxyLoginError(json.code)
							: json.message
					);
					throw error;
				}
...
function assignSithsCard({ username, password, timeout = 5000 } = {}) {
	return dispatch => {
		return fetch(
			`${CLIENT_STRATUM_SERVER}/api/authentication/assign?_=${new Date().getTime()}&username=${username}&password=${password}`,
			{
				credentials: 'include',
			}
		)
			...
			.then(json => {
				if (json.success) {
					...
					const errorCode =
						json.code === 1 && 'ASSIGN_WRONG_CREDENTIALS';
					const error = new Error(
						getStratumProxyLoginError(errorCode)
					);
					throw error;
				}
			})
...
export function logoutFromStratum() {
	return dispatch => {
		return fetch(
			`${CLIENT_STRATUM_SERVER}/api/authentication/logout?_=${new Date().getTime()}`,
			{
				credentials: 'include',
			}
		)
...
function getAvailableContexts(context, initial, isLogin) {
	return dispatch => {
		return fetch(
			`${CLIENT_STRATUM_SERVER}/api/authentication/contexts?_=${new Date().getTime()}`,
			{
				credentials: 'include',
			}
		)
			...
			.then(json => {
				if (json.success) {
					const contexts = json.data.filter(
						c =>
							c.Unit.Register.RegisterID ===
							parseInt(CLIENT_REGISTER_ID)
					);
					if (contexts.length <= 0) {
						...
						throw new Error(
							'Du har tyvärr inte tillgång till det här registret.'
						);
					} else {
						// Successful login
...
export function checkTimeleft(repeatAfter) {
	return dispatch => {
		const isActive =
			window.name && window.name === cookies.get(stratumSessionCookie);
		...
		return fetch(
			`${CLIENT_STRATUM_SERVER}/api/authentication/timeleft?_=${new Date().getTime()}`,
			{
				credentials: 'include',
			}
		)
			...
			.then(json => {
				if (json.success) {
					...
					if (timeleft > 0 && typeof repeatAfter === 'number') {
						setTimeout(
							() => dispatch(checkTimeleft(repeatAfter)),
							repeatAfter
						);
					}
				}
