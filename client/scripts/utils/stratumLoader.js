...
function getResource(aURL) {
	var sa = [].map
		.call(document.scripts, function(x) {
			return x.src;
		})
		.concat(
...
function loadEnvironment(aCallback) {
	...
	Ext.Ajax.request({
		url: '/stratum/api/configurations/globals',
		method: 'GET',
		success: function(r1) {
			...
			Ext.Ajax.request({
				...
				success: function(r2) {
					var o2 =
						r2.responseText && Ext.decode(r2.responseText).data;

					if (!o2) {
						// Not https or not authenticated.
					...
					Ext.Ajax.request({
						url:
							'/stratum/api/metadata/registers/' +
							o2.Unit.Register.RegisterID,
						method: 'GET',
						success: function(r3) {
							var o3 =
								r3.responseText &&
								Ext.decode(r3.responseText).data;
...
function inject(aResourceList, aReadyCallback, target) {
	...
	for (ia = 0; ia < aResourceList.length; ia++) {
		uc = aResourceList[ia];
		if (getResource(uc) && uc.indexOf('api/widgets') < 0) {
			// Do not not if already loaded, except for widgets.
			cc--;
			if (cc === 0) {
				aReadyCallback();
				return;
...
export function startRegistrations(
	...
	if (!pn) {
		callback({
			success: false,
			cancelled: true,
			error: `Kunde inte hitta element "${target}" för registreringsapplikationen.`,
		});
		return;
	}
		...
		pn.createChild(
			{
				...
				html:
					...
					(typeof aMessage === 'string'
						? aMessage
						: 'Du saknar behörighet att se denna sida. Har du loggat in till rätt register och vårdenhet?') +
					'</div>',
			},
			pn.first()
		);
	...
	loadEnvironment(function() {
		if (!Profile.Context) {
			Stratum.showError(
				'Du behöver vara inloggad för att arbeta med registreringar. Logga in först och försök igen.'
			);
			return;
		}
		/*
		...
		*/
		pn.createChild({
			id: 'TopbarPanel',
			style:
				'height: 48px; margin-bottom: 18px; padding: 6px 0px 4px 0px; border-top: 1px solid #ddd; border-bottom: 1px solid #ddd',
		});
		...
		window.onhashchange = function(anEvent, forceReload) {
			...
			switch (lc.hash) {
				...
				case 'page':
					...
					break;
				case '':
					Stratum.ApplicationForRegistrations('page', {
						id: Profile.Site.FirstPage.PageID,
						scope: 1,
					});
					break;
				default:
...
export function startWidget(target, widget, queryString, callback = () => {}) {
	...
	Ext.onReady(function() {
		loadEnvironment(function() {
			...
			if (targetNode) {
				inject(
					['/stratum/api/widgets/' + widget + queryString],
					callback
				);
			} else {
...
function injectWithCacheBusting(urisToInject, startFunction) {
	...
	xhttp.open(
		'GET',
		`/stratum/api/configurations/globals?_=${new Date().getTime()}`,
		true
	);
	xhttp.send();
}
	...
	const doInjects = () => {
		const startFn =
			widget === 'registrations'
				? () =>
						startRegistrations(target, (...args) => {
							resumeQueue();
							callback(...args);
						})
				: () =>
						startWidget(target, widget, queryString, (...args) => {
							resumeQueue();
							callback(...args);
						});
